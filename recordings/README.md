...
| 情境 | 障礙圓柱(不可見) | 箱型障礙(可見) | 站立人物 | 走動人物 |
|---|---|---|---|---|
| `static` | 啟用 | 啟用 | 在圓柱位置 | **站在 USD 原位、不走動** |
| `dynamic` | 全部關閉 | 全部關閉 | **仍在場** | ORCA 互動避讓 |
| `mixed` | 啟用 | 啟用 | 在圓柱位置 | ORCA 互動避讓 |

### ⚠⚠ `dynamic` 並不是「沒有靜態障礙」

`scenarios.py` 的註解寫 `dynamic` 是「行人沿路徑走動，靜態障礙全部關掉」，
圓柱確實全關（log 印 `0/18 啟用`）。**但 `run_isaac_sim` 的 `place_standing`
不看 `obstacles_enabled`**，照樣把 3~5 個站立人物擺到那些被關掉的圓柱位置上。
所以 `dynamic` 場上仍有 3~5 個**靜止的人形障礙**。

後果是 `dynamic` 與 `mixed` 的靜態內容幾乎一樣：
...
而且 person 圓柱是 `visibility=invisible`（只當物理碰撞體），箱型是可見的。
velodyne 是 RTX 光達、打的是**算圖網格**，所以那些圓柱本來就不在點雲裡；
圓柱又一律與站立人物同一位置（站立人物數 = person 圓柱數），
光達看得到的差別**只有那一個箱子**（第3、4趟）。

兩者仍有差：`mixed` 的圓柱是實體碰撞體（半徑 0.25），且 ORCA 在 `mixed`
會把圓柱當障礙餵進去、行人走法因此不同。但這不是「有/沒有靜態障礙」的對比。

**寫論文時不可以把 `dynamic` 描述成純動態場景。** 要嘛照實描述成
「無推車類障礙物、但有靜止行人」，要嘛把站立人物一起關掉重錄那 12 趟。
