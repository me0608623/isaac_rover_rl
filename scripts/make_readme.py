...
def render(runs) -> str:
    ...
    A("## 要挑影片看 → 先進 `00_影片總覽/`")
    A("")
    A("那裡面是**中文命名的符號連結**，檔名直接寫清楚是哪個模型、什麼情境、")
    A("第幾趟、場上幾個障礙幾個行人、哪個視角：")
    A("")
    A("```")
    A("00_影片總覽/")
    A("    01_主批次_三模型正式錄影/模型sa5r2/靜動態混合_第4趟_障礙6行人8_俯視.mp4")
    A("    02_對照_行人走固定路線/   03_對照_速度0.6/   04_對照_速度1.0/")
    A("    05_每趟原始資料_bag與CSV/模型sa5r2_靜動態混合_第4趟_障礙6行人8/")
    A("```")
    A("")
    A("| 中文 | 原文 | 意思 |")
    A("|---|---|---|")
    A("| 靜態障礙 | `static` | 有靜態障礙，行人站著不走 |")
    A("| 動態行人 | `dynamic` | 沒有靜態障礙，行人用 ORCA 互動避讓 |")
    A("| 靜動態混合 | `mixed` | 兩者都有 |")
    A("| 俯視 / 車後 / 斜前方 | `topdown` / `chase` / `oblique` | 三個機位 |")
    A("")
