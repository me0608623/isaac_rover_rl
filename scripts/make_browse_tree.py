...
def _counts(run_index: int):
    ...
    if run_index < 1:
        return (0, 0)
    v = variant(run_index)
    return (len(v.obstacles), len(v.walks))
    ...
    for run_dir, meta in _runs(root):
        ...
        for mp4 in sorted((run_dir / "video").glob("*.mp4")):
            ...
            _link(mp4, browse / MAIN_DIRNAME / f"模型{model}"
                  / video_name(scen, idx, cam, nobs, nwalk))
            n += 1
        _link(run_dir, browse / RAW_DIRNAME
              / f"模型{model}_{run_label(scen, idx, nobs, nwalk)}")
